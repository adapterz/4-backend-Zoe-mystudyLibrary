...
// 1-3. 특정 게시글 상세보기
export async function detailBoardModel(category, board_index, page, ip, user_index) {
...
// 2. 게시글 작성/수정/삭제
// 2-1. 게시글 최초 작성
export async function writeBoardModel(category, input_write, user_index, ip) {
  let query;
  // 게시글 작성 쿼리문
  query =
  ...
  try {
    ...
    if (tag_query !== "") await myPool.query(tag_query);
    // 성공 로그찍기, 커밋하기
...
export async function getWriteModel(board_index, user_index, ip) {
...
// 2-3. 게시글 수정 요청
export async function editBoardModel(input_write, board_index, user_index, ip) {
  // 게시글 정보 수정 요청 쿼리문
  let query =
...
// 2-4. 게시글 삭제 요청
export async function deleteBoardModel(board_index, user_index, ip) {
  // 해당 인덱스 게시글 삭제
  const query =
    // 게시글 삭제 쿼리문
...
// 3. 좋아요 요청/검색기능
// 3-1. 게시글 좋아요 요청
export async function favoriteBoardModel(board_index, user_index, ip) {
  ...
  try {
    ...
    // 쿼리문 실행
    await myPool.query(query, [board_index, user_index, moment().format("YYYY-MM-DD HH:mm:ss")]);
    // 성공 로그찍기
    await querySuccessLog(ip, query);
    // 정상적으로 좋아요 수 1증가
    await myPool.query("COMMIT");
...
// 3-2. 게시글 검색 기능
export async function searchBoardModel(search_option, search_content, category, page, ip) {
