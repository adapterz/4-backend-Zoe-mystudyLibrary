...
// 1. 회원가입/탈퇴
// 회원가입 약관확인
router.get("/sign-up/guide", signUpGuide);
...
// 회원탈퇴 요청
router.delete("/drop-out", body("checkBox").isBoolean(), isValidate, dropOut);

// 2. 로그인/로그아웃
// 로그인 요청
router.post("/login", body("id").isString().trim(), body("pw").isString().trim(), isValidate, login);
// 로그아웃 요청
router.post("/logout", logout);

// 3. 관심도서관 조회/등록/삭제
// 관심도서관 조회
router.get("/user-lib", userLibrary);
// 관심도서관 등록
router.patch("/user-lib", query("libraryIndex").isInt().isLength({ min: 1, max: 4 }), isExist, registerUserLibrary);
// 관심도서관 삭제
router.delete("/user-lib", query("libraryIndex").isInt().isLength({ min: 1, max: 4 }), isExist, deleteUserLibrary);

// 4. 유저가 작성한 글/댓글/후기
// 유저가 쓴 글 목록 가져오기
router.get("/post", checkPageValidation, userBoard);
// 유저가 쓴 댓글 목록 가져오기
router.get("/comment", checkPageValidation, userComment);
// 유저가 작성한 도서관 후기 목록 가져오기
router.get("/review", checkPageValidation, userReview);
