...
const detailBoard = async function (req, res) {
  ...
  // 로그인을 하지 않았을 때
  if (!login_cookie) {
    let model_results = await post_model.detailBoardModel(req.params.boardIndex, req.ip, null);
  }
  // 로그인을 했을 때
  if (login_cookie) {
    let model_results = await post_model.detailBoardModel(req.params.boardIndex, req.ip, login_cookie);
  }
  // mysql query 메서드 실패
  if (model_results.state === "mysql 사용실패") return res.status(500).json(model_results);
...
const writePost = async function (req, res) {
  ...
  if (req.params.category === "proof-shot") req_category = "공부인증샷";
  // 게시글 작성 모델 실행 결과 변수
  const model_results = await post_model.writePostModel(req_category, req.body, login_cookie, req.ip);
  // mysql query 메서드 실패
  if (model_results.state === "mysql 사용실패") return res.status(500).json(model_results);
  // 게시글 작성 요청 성공
...
const writeComment = async function (req, res) {
  ...
  if (!login_cookie) return res.status(401).json({ state: "해당 서비스 이용을 위해서는 로그인을 해야합니다." });
  // 댓글 작성 모델 실행 결과
  const model_results = await comment_model.writeCommentModel(req.params.boardIndex, login_cookie, req.ip);
  // mysql query 메서드 실패
  if (model_results.state === "mysql 사용실패") return res.status(500).json(model_results);
  // 성공적으로 댓글 작성 요청 수행
...
const reviseComment = async function (req, res) {
  ...
  if (!login_cookie) return res.status(401).json({ state: "해당 서비스 이용을 위해서는 로그인을 해야합니다." });
  // 해당 댓글에 대한 유저의 권한 체크
  const check_authority = await check_authority_model.isCommentAuthorModel(req.query.commentIndex, login_cookie, req.body, req.ip);
  // mysql query 메서드 실패
  if (check_authority.state === "mysql 사용실패") return res.status(500).json(check_authority);
  // 해당 commentIndex와 일치하는 로우가 없을 때
  else if (check_authority.state === "존재하지않는댓글") return res.status(404).json(check_authority);
  ...
  // 해당 게시물 작성한 유저와 로그인한 유저가 일치할 때
    ...
    // 댓글수정 모델 실행 결과
    const model_results = await comment_model.reviseCommentModel(req.query.commentIndex, login_cookie, req.ip);
    // mysql query 메서드 실패
    if (model_results.state === "mysql 사용실패") return res.status(500).json(model_results);
    else if (model_results.state === "존재하지않는댓글") return res.status(404).json(model_results);
    // 성공적으로 댓글삭제 요청 수행
...
const searchPost = async function (req, res) {
   ...
   */
  // 검색 모델 실행 결과
  const model_results = await post_model.searchModel(req.query.searchOption, req.query.searchContent, req.params.cateogry, req.ip);
  // mysql query 메서드 실패
  if (model_results.state === "mysql 사용실패") return res.status(500).json(model_results);
  // 검색결과가 없을 때
