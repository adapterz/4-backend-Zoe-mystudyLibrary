...
const registerComment = async function (req, res) {
  ...
  if (!login_cookie) return res.status(401).json({ state: "해당 서비스 이용을 위해서는 로그인을 해야합니다." });
  // 후기 등록 모델 사용 결과
  const model_results = await review_model.registerReviewModel(req.params.libIndex, login_cookie, req.body, req.ip);
  // mysql query 메서드 실패
  if (model_results.state === "mysql 사용실패") return res.status(500).json(model_results);
  // 성공적으로 도서관 후기 등록
...
const reviseReview = async function (req, res) {
  ...
  if (!login_cookie) return res.status(401).json({ state: "해당 서비스 이용을 위해서는 로그인을 해야합니다." });
  // 해당 후기에 대한 유저의 권한 체크
  const check_authority = await check_authority_model.isReviewAuthorModel(req.query.reviewIndex, login_cookie, req.body, req.ip);
  // mysql query 메서드 실패
  if (check_authority.state === "mysql 사용실패") return res.status(500).json(check_authority);
  // 해당 commentIndex와 일치하는 로우가 없을 때
  else if (check_authority.state === "존재하지않는후기") return res.status(404).json(check_authority);
