...
async function allLibModel(ip) {
  ...
  const query =
    "SELECT libraryIndex,libraryName,libraryType,closeDay,openWeekday,endWeekday,openSaturday,endSaturday,openHoliday,endHoliday,nameOfCity,districts,address,libraryContact,COUNT(grade),AVG(grade) FROM LIBRARY INNER JOIN REVIEW ON LIBRARY.libraryIndex=REVIEW.libraryIndex WHERE LIBRARY.deleteDateTime IS NULL AND REVIEW.deleteDateTime IS NULL GROUP BY libraryIndex";
  try {
    // 쿼리문 메서드 성공
    const [results, fields] = await db.pool.query(query);
    // 성공 로그찍기
...
async function localLibModel(input_local, ip) {
  ...
  const query =
    "SELECT libraryIndex,libraryName,libraryType,closeDay,openWeekday,endWeekday,openSaturday,endSaturday,openHoliday,endHoliday,nameOfCity,districts,address,libraryContact,AVG(grade) FROM LIBRARY INNER JOIN REVIEW ON LIBRARY.deleteDateTime=REVIEW.deleteDateTime WHERE LIBRARY.deleteDateTime IS NULL AND REVIEW.deleteDateTime IS NULL AND nameOfCity =" +
...
async function particularLibModel(library_index, ip) {
  ...
  const query =
    "SELECT libraryIndex,libraryName,libraryType,closeDay,openWeekday,endWeekday,openSaturday,endSaturday,openHoliday,endHoliday,nameOfCity,districts,address,libraryContact FROM LIBRARY WHERE deleteDateTime IS NULL AND libraryIndex=" +
    mysql.escape(library_index) +
    ";" +
    +"SELECT reviewIndex,libraryIndex,userIndex,reviewContent,grade,createDateTime,COUNT(grade),AVG(grade) FROM REVIEW WHERE deleteDateTime IS NULL AND libraryIndex =" +
    mysql.escape(library_index) +
    "GROUP BY libraryIndex;";
