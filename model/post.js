...
// 특정 게시글 상세보기
function detailBoardModel(board_index, ip, user_index) {
  ...
  db.db_connect.query(query, function (err, results) {
    ...
    if (user_index !== null) {
      ...
      view_query =
        "SELECT userIndex FROM VIEWPOST WHERE deleteDateTime IS NULL AND userIndex =" +
        mysql.escape(user_index) +
        ";" +
        "SELECT ip FROM VIEWPOST WHERE deleteDateTime IS NULL AND ip =" +
        mysql.escape(ip) +
        ";";
    }
    // 로그인 안한 유저가 글 조회했을 때
      ...
      view_query = "SELECT ip FROM VIEWPOST WHERE deleteDateTime IS NULL AND ip =" + mysql.escape(ip);
    }
...
function revisePost(input_post, board_index, user_index, ip) {
  ...
  db.db_connect.query(query, function (err, results) {
    ...
    querySuccessLog(ip, query);
    let tag_query = "";
    // 태그 쿼리문 추가( req.body.tags 배열이 비어있으면 해당 반복문은 실행되지 않음)
