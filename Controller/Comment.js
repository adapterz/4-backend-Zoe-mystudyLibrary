...
const writeComment = async function (req, res) {
  ...
  // 성공적으로 댓글 작성 요청 수행
  else if (model_results.state === "댓글작성") return res.status(CREATED).end();
};
// 수정시 기존 댓글 정보 불러오기
const getComment = async function (req, res) {
  ...
  // 게시글이 존재하지 않을 때
  else if (check_comment.state === "존재하지않는게시글") return res.status(NOT_FOUND).json(check_comment);
  // 댓글이 존재하지 않을 때
  ...
  // 해당 게시물 작성한 유저와 로그인한 유저가 일치할 때
    ...
    if (model_results.state === "mysql 사용실패") return res.status(INTERNAL_SERVER_ERROR).json(model_results);
    // 해당 댓글이 존재하지 않을 때
    else if (model_results.state === "존재하지않는댓글") return res.status(NOT_FOUND).json(model_results);
    // 성공적으로 댓글 정보 가져왔을 때
    else if (model_results.state === "댓글정보로딩") return res.status(OK).json(model_results.data);
  }
};
// 댓글 수정 요청
const reviseComment = async function (req, res) {
  ...
  // 해당 댓글 작성한 유저와 로그인한 유저가 일치할 때
    ...
    if (model_results.state === "mysql 사용실패") return res.status(INTERNAL_SERVER_ERROR).json(model_results);
    // 성공적으로 댓글수정 요청 수행
    else if (model_results.state === "댓글수정") return res.status(OK).end();
  }
};
// 댓글 삭제
