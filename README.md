...
1. 위에서 언급했듯이 게시글 중복 조회 여부는 쿠키로 발급합니다. 현재 게시글 중복 조회 여부를 확인해주는 세부적인 방식에 대해서 언급하겠습니다. 게시글을 조회할 때마다 기존의 쿠키는 없애고 새 게시글 인덱스의 정보를 추가한 값을 가진 쿠키를 새로 발급하도록 해줬습니다. 즉 새 게시글을 조회할 때마다 쿠키의 만료기간이 갱신되는데 이를 해결하기 위한 고민이 필요할 것 같습니다.

2. 도서관 관련 데이터를 응답해줄 때 초기 기획 상 페이지네이션을 하지 않았습니다. 그런데 프론트 프로젝트를 진행하면서 기획을 무한 스크롤링으로 변경했는데 이에 맞춰 페이지네이션을 해주고 싶습니다. 

3. 현재 DB의 로그 관련 테이블에 어떤 요청과 응답이 오갔는지만 저장을 합니다. 다음번에는 에러 핸들러에 걸리는 오류들도 저장해주면 좋을 것 같다는 생각을 했습니다.
