...
1. 프로젝트가 거의 마무리 됐을 당시에 REST API에 대해 알게됐습니다. 행위는 HTTP 메서드로, uri는 자원으로 표기해야한다는 점을 알게됐습니다. 현재 uri가 들어가야하는 항목에 행위를 의미하는 정보들이 포함돼 있는 점을 보완하고 싶습니다.
+ 추가

2. 위에서 언급했듯이 게시글 중복 조회 여부는 쿠키로 발급합니다. 현재 게시글 중복 조회 여부를 확인해주는 세부적인 방식에 대해서 언급하겠습니다. 게시글을 조회할 때마다 기존의 쿠키는 없애고 새 게시글 인덱스의 정보를 추가한 값을 가진 쿠키를 새로 발급하도록 해줬습니다. 즉 새 게시글을 조회할 때마다 쿠키의 만료기간이 갱신됩니다. 만약 어떤 유저가 매일 만료기간이 지나기 전에 새로운 게시글을 계속 조회하게 되면 언젠가는 쿠키의 값이 하나의 쿠키에 저장할 수 있는 최대 용량이 될 것입니다. 이를 방지하기 위한 고민을 추가로 해보고 싶습니다.

3. 도서관 관련 데이터를 응답해줄 때 초기 기획 상 페이지네이션을 하지 않았습니다. 그런데 프론트 프로젝트를 진행하면서 기획을 무한 스크롤링으로 변경했는데 이에 맞춰 페이지네이션을 해주고 싶습니다. 

4. 현재 DB의 로그 관련 테이블에 어떤 요청과 응답이 오갔는지만 저장을 합니다. 다음번에는 에러 핸들러에 걸리는 오류들도 저장해주면 좋을 것 같다는 생각을 했습니다.

5. controller에서 로그인을 했는지 여부가 체크가 필요한 기능인 경우 로그인 체크 여부가 매번 똑같은 코드로 반복됩니다. 마찬가지로 controller의 에러 핸들러에서도 거의 비슷한 코드들이 반복됩니다. 이 코드를 따로 메서드로 정의하면 좋을 것 같다는 생각을 했습니다.
